...
struct BeatScoreInfo
{
	int beatIndex;
	int noOfNoteInBeats;
	std::vector<DTWDetail> dtwDetails;
	double dist;
};
...
static inline double min (double x, double y ) { return x < y ? x : y;}
    ...
    dtw(uint n, uint constraint): mGamma(n,vector<double>(n,INF)),mN(n),mM(n),mConstraint(constraint) {    	
    }
    dtw(uint n, uint m, uint constraint): mGamma(n,vector<double>(m,INF)),mN(n),mM(m),mConstraint(constraint) {    	
    }
    /*
    ...
    */
    ...
    inline double fastdynamicDetail(const vector<double> &  v, const vector<double> & w, vector<uint32_t> beats, vector<DTWDetail>& distPerSample, vector<BeatScoreInfo>& scorePerBeat) {
    	if(! fast) return dynamic(v,w,mConstraint,1);
    	assert(static_cast<int>(v.size()) == mN);
    	assert(static_cast<int>(w.size()) == mM);
    	assert(static_cast<int>(mGamma.size()) == mN);
    	double Best(INF);
        for (int i = 0; i < mN;++i) {
        	assert(static_cast<int>(mGamma[i].size()) == mM);
            for (int j = max(0,i-mConstraint); j < min(mM,i+mConstraint+1);++j) {
                Best = INF;
                if(i>0) Best = mGamma[i-1][j];
                if(j>0) Best = min(Best, mGamma[i][j-1]);
                if( (i > 0) && (j > 0) )
	...
	for (int k = 1; k < mN; k++)
	{
		if (mGamma[k][mM-1] < mGamma[min][mM-1])
		{
			min = k;	
		}
	...
	printf ("-------------------------------------------------------------------\n");	
	int beatI = 0;
	vector<DTWDetail>::iterator it = distPerSample.begin ();
	//int vectorSize = mN;
	//if (vectorSize < mM) vectorSize = mM;
	//distPerSample.reserve (vectorSize);	
	while (p >= 0 && q >= 0)
	{
	    char* token = ",";
	    int pt = p;
	    ...
	    double c = MAX_DIST;
	    double d = 0;
	    if (p > 0 && q > 0)  a = mGamma[p-1][q-1];
	    if (p > 0)  b = mGamma[p-1][q];
	    if (q > 0)  c = mGamma[p][q-1];
	    if (b < a)
	    {
		if (c < b)
		{
	    ...
	    printf ("%3d,%3d,%05.3f%s",pt,qt,d,token);	
	    DTWDetail dtwDetail;
	    dtwDetail.refX = pt;
	    dtwDetail.testX	 = qt;
	    dtwDetail.dist	 = d;
	    //distPerSample[vectorSize-1] = dtwDetail;
	    //vectorSize--;
	    distPerSample.insert (it,dtwDetail);
	    it = distPerSample.begin ();
#if 0
	    if (scoreDetail->refX == pt)
	    {
		scoreDetail->score += d;
	    	scoreDetail->testX.push_back (qt); 
	    } else
	    {
		scorePerFrame.push_back (*scoreDetail);
		delete scoreDetail;
	...
	int notes = 0;
	int beatIndex = 0;
	BeatScoreInfo beatScoreInfo;
	beatScoreInfo.beatIndex = 0;
	beatScoreInfo.noOfNoteInBeats = beats [0];
	beatScoreInfo.dist = 0;
	int prevX = distPerSample[0].refX;
	for (uint32_t i = 0; i < distPerSample.size (); i++)
	{
		if (distPerSample[i].refX != prevX)
		{
			notes++;
			prevX = distPerSample [i].refX;
		} else
		{
		}
		if (notes <  beats [beatIndex])
		{
			//Within
		} else 
		{
			scorePerBeat.push_back (beatScoreInfo);
			notes = 0;
			beatIndex++;
			beatIndex = beatIndex % beats.size ();
			beatScoreInfo.beatIndex = beatIndex;
			beatScoreInfo.noOfNoteInBeats = beats [beatIndex];
			beatScoreInfo.dist = 0;
			beatScoreInfo.dtwDetails.clear ();
		}
		beatScoreInfo.dtwDetails.push_back (distPerSample[i]);	
		beatScoreInfo.dist += distPerSample[i].dist;
		if  ((i+1) == distPerSample.size ())
		{
			scorePerBeat.push_back (beatScoreInfo);
		}
	}

	return mGamma[mN-1][mM-1];
    }

    vector<vector<double> > mGamma;

    int mN,mM, mConstraint;
